...
test.describe('Flow Navigator - JointJS core-only mode', () => {
  test('minimap is disabled by default because JointJS+ Navigator is not available', async ({ page }) => {
    await login(page);
    ...
    await expect(page.locator('.flow-builder__toolbar')).not.toContainText('shared.flowBuilder.toolbar.navigator');
  });
});
