...
describe('workflowStudioRoutes', () => {
  it('registers lifecycle routes and protects the dirty builder', () => {
    ...
    expect(
      workflowStudioRoutes.find((route) => route.path === 'ai-agent-mcrs/workflows/create')
        ?.canDeactivate,
    ).toContain(serviceManagementUnsavedChangesGuard);
    expect(
      workflowStudioRoutes.find(
        (route) => route.path === 'ai-agent-mcrs/workflows/:workflowId/edit',
      )?.canDeactivate,
    ).toContain(serviceManagementUnsavedChangesGuard);
  });
});
