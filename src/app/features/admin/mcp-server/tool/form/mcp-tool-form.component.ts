...
import { McpCategoryResponse, McpCollectionField, McpDbConfig, McpDbQueryType, McpEndpointConfig, McpEndpointMethod, McpToolCreateDto, McpToolResponse, McpToolType, McpToolUpdateDto } from '../../../../../core/models/mcp-server/mcp-tool.model';
...
export class McpToolFormComponent implements OnInit, OnDestroy {
  ...
  formInitialValue: McpToolCreateDto = { ...MCP_TOOL_INITIAL_VALUE };
  submitting = false;
  loading = false;
  private currentModel: McpToolCreateDto = { ...MCP_TOOL_INITIAL_VALUE };
  ...
  onFormValueChange(rawModel: Record<string, unknown>): void {
    const nextModel = rawModel as unknown as McpToolCreateDto;
  ...
  onSubmitForm(model: McpToolCreateDto): void {
    if (this.submitting) {
      return;
    }

    const type = model.type ?? 'endpoint';
    model.category = model.category ?? this.categories[0]?.code ?? this.defaultCategoryCode;
    model.tags = this.normalizeTags(model.tags);
    model.endpoint = type === 'endpoint' ? this.normalizeEndpoint(model.endpoint) : undefined;
    model.db = type === 'db' ? this.normalizeDb(model.db) : undefined;

    const request$ = this.toolId ? this.toolService.update(this.toolId, model as McpToolUpdateDto) : this.toolService.create(model);
  ...
  private mapToInitialValue(tool: McpToolResponse): McpToolCreateDto {
  ...
  private createInitialValue(): McpToolCreateDto {
    return {
      ...MCP_TOOL_INITIAL_VALUE,
