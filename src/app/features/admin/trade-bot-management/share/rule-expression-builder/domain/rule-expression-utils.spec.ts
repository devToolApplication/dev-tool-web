...
import {
  ...
  extractRuleExpressionDependencies,
} from './rule-expression-dependencies';
import {
...
describe('rule expression utilities', () => {
  it('prints nested groups and skips disabled nodes', () => {
    const expression = {
      root: createRuleExpressionGroup(
        'OR',
        [
          createRuleExpressionCondition({
            id: 'condition-1',
  ...
  it('validates group, not, xor, default condition and range constraints', () => {
    expect(validateRuleExpression({ root: null }).errors[0].message).toBe(
      'tradeBot.ruleExpression.validation.rootRequired',
    );
    ...
    expect(validateRuleExpression({ root: defaultCondition }).errors[0].message).toBe(
      'tradeBot.ruleExpression.validation.operandRequired',
    );

    const xor = createRuleExpressionGroup('XOR', [createRuleExpressionRuleRef('A')], { id: 'xor' });
    expect(validateRuleExpression({ root: xor }).errors[0].message).toBe(
      'tradeBot.ruleExpression.validation.xorChildCount',
    );

    const not = createRuleExpressionNot([], { id: 'not' });
    expect(validateRuleExpression({ root: not }).errors[0].message).toBe(
      'tradeBot.ruleExpression.validation.notChildCount',
    );
    ...
    expect(validateRuleExpression({ root: range }).errors[0].message).toBe(
      'tradeBot.ruleExpression.validation.rangeOrder',
    );
  });
  ...
  it('allows rule references for crossover operands because runtime compares rule values', () => {
    ...
    expect(
      validateRuleExpression({ root: expression }).errors.some(
        (error) => error.message === 'tradeBot.ruleExpression.validation.incompatibleOperand',
      ),
    ).toBe(false);
    ...
    expect(
      validateRuleExpression({ root: ruleToRule }).errors.some(
        (error) => error.message === 'tradeBot.ruleExpression.validation.incompatibleOperand',
      ),
    ).toBe(false);
  });

  it('validates rule references structurally when no rule catalog is provided', () => {
    const result = validateRuleExpression({
      root: createRuleExpressionRuleRef('TREND_FILTER', { id: 'rule-ref' }),
    });
    ...
    const expression = {
      root: createRuleExpressionGroup(
        ...
        [
          ...
          createRuleExpressionRuleRef('CONFIRM_VOLUME', {
            id: 'rule-ref',
            slotCode: 'confirm',
            params: { lookback: 10 },
          }),
  ...
  it('detects inactive, self and circular rule references', () => {
    ...
    expect(
      circular.errors.some((error) => error.message.includes('CURRENT -> CHILD -> CURRENT')),
    ).toBe(true);
