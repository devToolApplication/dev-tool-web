...
import {
  StrategyResponse,
  SymbolResponse,
} from '../../../../../../core/models/trade-bot/reference-data.model';
import {
...
import {
  STRATEGY_MANAGEMENT_ROUTES,
  STRATEGY_FAMILY_LABELS,
} from '../../strategy-management.constants';
import {
  StrategyFormBuilders,
  StrategyGeneralInfoFormGroup,
} from '../../shared/strategy-form.builders';
import {
...
} from '../../shared/strategy-form.facade';
...
export class StrategyFormPageComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.generalInfoForm.controls.symbolId.valueChanges
  ...
  get routePathPreview(): string {
    ...
    return this.editId
      ? STRATEGY_MANAGEMENT_ROUTES.edit(this.editId)
      : STRATEGY_MANAGEMENT_ROUTES.createByPath(this.strategyMeta.routePath);
  }
  ...
  get selectedRuleDetails(): Array<{
    slot: StrategyRuleSlotDefinition;
    rule?: StrategyRuleResponse;
    ...
    return this.strategyDefinition.ruleSlots.map((slot) => ({
      ...
      configJson: this.findRuleById(this.ruleSlotsForm.controls[slot.slotCode]?.value ?? '')
        ?.configJson,
    }));
  }
  ...
  save(): void {
    ...
    if (
      ...
      this.ruleSlotsForm.invalid ||
      this.strategyConfigSection?.isValid() === false
    ) {
      ...
      .subscribe({
        next: (binding) => {
          this.toastService.success(
            this.i18nService.t(
              this.editId
                ? TradeBotTextKey.UpdateStrategySuccess
                : TradeBotTextKey.SaveStrategySuccess,
            ),
          );
        ...
        error: (error) =>
          this.toastService.error(
            error?.error?.errorMessage ?? this.i18nService.t(TradeBotTextKey.SaveStrategyFailed),
          ),
      });
  }
  ...
  private getRuleOptionsForSlot(
    slot: StrategyRuleSlotDefinition,
    pinnedRuleGroupCode?: string | null,
  ): StrategySelectOption[] {
    const acceptedRuleCodes = new Set(
      (slot.acceptedRuleCodes ?? []).map((item) => item.toUpperCase()),
    );
    const acceptedRuleGroupCodes = new Set(
      (slot.acceptedRuleGroupCodes ?? []).map((item) => item.toUpperCase()),
    );
    const effectiveRuleGroupCode =
      pinnedRuleGroupCode ?? this.getPinnedRuleGroupCode(slot.slotCode);

    return this.availableRules
      .filter((item) => item.status === 'ACTIVE')
      .filter(
        (item) =>
          acceptedRuleCodes.size === 0 ||
          acceptedRuleCodes.has(
            String(item.code ?? '')
              .trim()
              .toUpperCase(),
          ),
      ...
      .filter(
        (item) =>
          !effectiveRuleGroupCode ||
          this.normalizeRuleGroupCode(item.ruleGroupCode) === effectiveRuleGroupCode,
      )
  ...
  private loadPage(data: Data, params: ParamMap): void {
    ...
    const request$ = editId
      ? this.facade.loadEditContext(editId)
      : this.facade.loadCreateContext(strategyServiceName);
    this.loadingService
      .track(request$)
      .pipe(
  ...
  private applyContext(
    context: StrategyFormPageContext,
    requestedStrategyServiceName: string,
  ): void {
    ...
    const strategyServiceName =
      context.binding?.strategyServiceName ?? requestedStrategyServiceName;
    this.currentStrategy =
      context.selectedStrategy ??
      context.references.strategies.find((item) => item.serviceName === strategyServiceName);
    ...
    this.buildRuleSlotsForm(
      this.strategyDefinition.ruleSlots,
      context.binding?.selectedRules ?? [],
    );
  ...
  private buildRuleSlotsForm(
    slots: StrategyRuleSlotDefinition[],
    selectedRules: TradeStrategySelectedRule[],
  ): void {
    const controls: Record<string, FormControl<string>> = {};

    slots.forEach((slot) => {
      const matchedSelection = selectedRules.find(
        (item) => item.slotCode?.toUpperCase() === slot.slotCode.toUpperCase(),
      );
      const defaultOption = this.getRuleOptionsForSlot(slot)[0]?.value ?? '';
      controls[slot.slotCode] = new FormControl(
        matchedSelection?.ruleId ?? (slot.required ? defaultOption : ''),
        {
          nonNullable: true,
  ...
  private patchEditState(
    ...
    this.strategyConfigInitialValue = mapApiConfigToStrategyConfig(
      binding.configJson ?? {},
      this.strategyDefinition,
    );
    this.strategyConfigModel = { ...this.strategyConfigInitialValue };
  }
  ...
  private buildPayload(): TradeStrategyBindingCreateDto {
    ...
    const strategyConfig =
      this.strategyConfigSection?.getModel<Record<string, unknown>>() ?? this.strategyConfigModel;
  ...
  private reconcileRuleGroupSelections(changedSlotCode?: string): void {
    if (
      this.strategyDefinition.ruleGroupSelectionMode !==
      StrategyFormPageComponent.SAME_GROUP_RULE_SELECTION_MODE
    ) {
      return;
    }
    ...
    const pinnedRuleGroupCode =
      (changedSlotCode
        ? this.normalizeRuleGroupCode(
            this.findRuleById(this.ruleSlotsForm.controls[changedSlotCode]?.value ?? '')
              ?.ruleGroupCode,
          )
  ...
  private defaultTradeSideMode(strategyServiceName: string): TradeSideMode {
    const normalized = String(strategyServiceName ?? '')
      .trim()
      .toUpperCase();
    if (normalized.endsWith('_BUY') || normalized.endsWith('_LONG')) {
      return 'LONG_ONLY';
    }
    if (normalized.endsWith('_SELL') || normalized.endsWith('_SHORT')) {
      return 'SHORT_ONLY';
    }
    return 'BOTH';
  }
  ...
  private prefillProviderSymbol(symbolId: string): void {
    ...
    if (
      !this.generalInfoForm.controls.providerSymbol.dirty ||
      !this.generalInfoForm.controls.providerSymbol.value
    ) {
      this.generalInfoForm.controls.providerSymbol.setValue(symbol.providerSymbol ?? symbol.code, {
        emitEvent: false,
      });
    }
