...
import { AiAgentModelConfigResponse } from '../../../../../core/models/ai-agent/ai-agent-model.model';
...
import { AI_AGENT_MODEL_ROUTES } from '../ai-agent-model.constants';

@Component({
  selector: 'app-ai-agent-model-list',
  standalone: false,
  templateUrl: './ai-agent-model-list.component.html'
})
export class AiAgentModelListComponent extends BasePagedList<AiAgentModelConfigResponse> implements OnInit {
  readonly tableConfig: TableConfig = {
    title: 'systemManagement.aiAgentModel.list.title',
    stateKey: 'system-management.ai-agent-models',
    emptyTitle: 'shared.table.emptyTitle',
    emptyDescription: 'shared.table.emptyDescription',
    errorTitle: 'loadError',
    toolbar: {
      new: { visible: true, label: 'systemManagement.action.newModel', icon: 'pi pi-plus', severity: 'success' },
  ...
  onCreate(): void {
    void this.router.navigate([AI_AGENT_MODEL_ROUTES.create]);
  }

  private goEdit(id: string): void {
    void this.router.navigate([`${AI_AGENT_MODEL_ROUTES.list}/edit`, id]);
  }
  ...
  protected loadPage(): void {
    this.runPageRequest(this.loadingService.track(this.service.getPage(this.page, this.pageSize, this.sorts, this.filters)), {
      errorMessage: 'systemManagement.aiAgentModel.toast.loadListFailed',
      onError: () => this.toastService.error('systemManagement.aiAgentModel.toast.loadListFailed')
    });
  }
}
