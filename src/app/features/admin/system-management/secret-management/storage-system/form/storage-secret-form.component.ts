...
export class StorageSecretFormComponent implements OnInit {
  ...
  private loadOptions(): void {
    ...
    this.loadingService.track(
      this.service.getAll().pipe(
        catchError(() => {
          this.toastService.error(this.i18nService.t('systemManagement.storageSecret.toast.loadCategoriesError') || 'Failed to load category options');
          return of([] as StorageSecretResponse[]);
        })
      )
  ...
  onSubmitForm(model: any): void {
    ...
    const request$ = this.editId
      ? this.service.update(this.editId, payload as StorageSecretUpdateDto)
      : this.service.create(payload as StorageSecretCreateDto);
    ...
    this.loadingService.track(request$).pipe(finalize(() => this.loading.set(false))).subscribe({
      ...
      error: (err) => {
        const errorMsg = err?.error?.message || err?.message || this.i18nService.t('systemManagement.storageSecret.toast.saveError');
  ...
  private applyRouteMode(id: string | null): void {
    ...
    this.loadingService.track(this.service.getById(id)).pipe(finalize(() => this.loading.set(false))).subscribe({
      ...
      error: (err) => {
        const errorMsg = err?.error?.message || err?.message || this.i18nService.t('systemManagement.storageSecret.toast.loadDetailError');
        this.toastService.error(errorMsg);
        void this.router.navigate([STORAGE_SECRET_ROUTES.list]);
      }
    });
