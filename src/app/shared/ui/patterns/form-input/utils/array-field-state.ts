...
export function createArrayFieldState<TModel extends object>(
  ...
  return {
    ...
    setValue: (val: unknown[]) => {
      modelSignal.update((m) => updateArrayValue(m, path, val));
      dirty.set(true);
    },
    touched,
    ...
    valid: computed(() => {
      ...
      return !errors() && children().every((group) => group.every((f) => f.valid()));
    }),
    markAsTouched() {
      touched.set(true);
    },
    ...
    arrayState: {
      addItem() {
        const defaultItem = config.itemConfig.reduce((acc: Record<string, unknown>, field) => {
          acc[field.name] = null;
          return acc;
        }, {});
...
function updateArrayValue<TModel extends object>(
  model: TModel,
  path: string,
  value: unknown[],
): TModel {
