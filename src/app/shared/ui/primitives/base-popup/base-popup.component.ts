...
@Component({
  ...
  template: `
    @if (visible) {
      <div
        class="fixed inset-0 z-[2400] flex items-center justify-center"
      ...
      >
        ...
        >
          <!-- Header -->
          <header
            class="flex items-center justify-between gap-3 border-b border-[var(--app-border-soft)] px-5 py-3.5"
          >
            <div class="min-w-0 flex-1">
              <ng-content select="[popup-header]"></ng-content>
              @if (!hasProjectedHeader) {
                <div class="flex flex-col gap-0.5">
                  <span class="text-lg font-semibold text-[var(--app-text)]">{{
                    header | translateContent
                  }}</span>
                  @if (subheader) {
                    <small class="text-[var(--app-text-muted)]">{{
                      subheader | translateContent
                    }}</small>
                  }
                </div>
              }
            ...
            <div class="flex items-center gap-2">
              ...
              @if (maximizable) {
                <button
                  type="button"
                  class="rounded-md p-1.5 text-[var(--app-text-muted)] border border-[var(--app-border-soft)] bg-[var(--app-surface-soft)] hover:bg-[var(--app-surface-alt)] hover:text-[var(--app-text)] transition-colors"
          ...
          @if (showFooter) {
            <footer
              class="flex items-center justify-between gap-3 border-t border-[var(--app-border-soft)] px-5 py-3.5"
            >
              ...
              <div class="flex items-center gap-2 ml-auto">
                @if (showDefaultCancel) {
                  <button
                    type="button"
                    class="rounded-lg px-4 py-2 text-sm font-medium text-[var(--app-text-soft)] hover:text-[var(--app-primary)] hover:bg-[var(--app-chart-primary-fill)] transition-all"
                    [disabled]="loading"
                    (click)="cancel.emit()"
                  >
